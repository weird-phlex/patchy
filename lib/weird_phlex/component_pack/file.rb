    ...
    class File
      ...
      def initialize(path, component_path:)
        ...
        if ['version.rb', 'Rakefile'].include?(split.first)
          ...
          @part = nil
          @file = split.first
        elsif (matches = @relative_path.match(%r{\Ashared/(?<subdirectory>(?:[^/]+/)*)(?<component>_.*_)/(?<part>[^/]*)/(?<file>.*)}))
          @shared_file = true
          @component = nil
          @part = matches[:part]
          @file = matches[:file]
        elsif (matches = @relative_path.match(%r{\A_global_/(?<part>[^/]*)/(?<file>.*)}))
          @shared_file = true
          @component = nil
          @part = matches[:part]
          @file = matches[:file]
        elsif (matches = @relative_path.match(%r{\Acomponents/(?<subdirectory>(?:[^/]+/)*)(?<component>_.*_)/(?<part>[^/]*)/(?<file>.*)}))
          @shared_file = false
